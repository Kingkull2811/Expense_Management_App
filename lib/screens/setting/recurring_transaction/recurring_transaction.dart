...
import 'recurring_info/recurring_info_bloc.dart';
import 'recurring_info/recurring_info_event.dart';
import 'recurring_transaction_bloc.dart';
...
class _RecurringPageState extends State<RecurringPage> {
  ...
  TransactionDataType _transactionTypeSelected = TransactionDataType(
    name: 'Giao dịch chi',
              ...
              final bool result = await Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => BlocProvider<RecurringInfoBloc>(
                    create: (context) =>
                        RecurringInfoBloc(context)..add(RecurringInfoInit()),
    ...
    return BlocConsumer<RecurringTransactionBloc, RecurringTransactionState>(
      ...
      builder: (context, state) {
        return state.isLoading
            ...
            : _listView(context, state.listRecurring);
      },
    );
  }
  ...
  Widget _listView(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) =>
          _createItemRecurring(context, listRecurring[index]),
    );
  }

  Widget _createItemRecurring(
    BuildContext context,
    RecurringListModel recurring,
  ) {
    Frequency frequency =
        getFrequencyByType(recurring.frequencyType ?? FrequencyType.daily);
    List<DayOfWeek> listDay =
        getDayOfWeekListFromStrings(recurring.dayInWeeks ?? []);
        ...
        child: ListTile(
          onTap: () async {
            final bool result = await Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => BlocProvider<RecurringInfoBloc>(
                  create: (context) => RecurringInfoBloc(context)
                    ..add(
                      RecurringInfoInit(),
                    ),
                  child: RecurringInfo(
          ...
          children: listTransactionType.map((transactionType) {
            return Container(
              ...
              child: ListTile(
                ...
                trailing:
                    (_transactionTypeSelected.type == transactionType.type)
                        ? Icon(
                            Icons.check,
                            size: 16,
                            color: Theme.of(context).primaryColor,
...
List<TransactionDataStatus> listTransactionStatus = [
  ...
  TransactionDataStatus(
    name: 'Sắp diễn ra',
    status: TransactionStatus.up_coming,
  ),
  TransactionDataStatus(
    name: 'Đã kết thúc',
