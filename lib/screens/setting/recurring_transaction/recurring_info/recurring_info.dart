...
import '../../../new_collection/new_collection.dart';
import '../../../new_collection/option_category/option_category.dart';
import '../../../new_collection/option_category/option_category_bloc.dart';
import 'recurring_info_bloc.dart';
...
class _RecurringInfoState extends State<RecurringInfo> {
  ...
  void initWhenEdit() {
    setState(() {
      ...
      toDate = isNotNullOrEmpty(toDate)
          ? getDateTimeFormat((widget.recurringListModel?.toDate)!)
          : null;
      itemCategorySelected = ItemCategory(
        ...
        type: widget.recurringListModel?.transactionType ??
      ...
      _moneyController.text =
          widget.recurringListModel?.amount.toString() ?? '';
      _noteController.text =
          widget.recurringListModel?.description.toString() ?? '';
  ...
  void initOptionTitle() {
    ...
    String frequencyName = (frequencyType == FrequencyType.weekday)
        ? dayWeek
        : getTitleByFrequencyType(frequencyType);
  ...
  @override
  void initState() {
    _recurringInfoBloc = BlocProvider.of<RecurringInfoBloc>(context)
      ..add(RecurringInfoInit());
          ...
          children: <Widget>[
            ...
            widget.isEdit
                ? _buttonDeleteUpdate(
                    context,
                    walletId,
                    itemCategorySelected.categoryId,
                  )
                : _buttonSave(
                    context,
                    walletId,
  ...
  Widget _buttonSave(BuildContext context, int? walletID, int? categoryID) {
    return Padding(
      ...
      child: PrimaryButton(
        text: 'Lưu',
        onTap: () async {
          await handleButtonSave(context, walletID, categoryID);
        },
      ),
    );
  }
            ...
            onTap: () async {
              if (_moneyController.text.isEmpty) {
                ...
                showMessage1OptionDialog(
                    context, 'Vui lòng chọn thời gian lặp lại');
              } else {
                List<String> enList =
                ...
                final Map<String, dynamic> data = {
                  ...
                  "toDate": toDate,
                  "transactionType":
                      itemCategorySelected.type?.name.toUpperCase(),
                ...
                if (widget.recurringListModel?.id != null) {
                  ...
                  if (response is RecurringPost) {
                    ...
                    showMessage1OptionDialog(
                        this.context, 'Cập nhật giao dịch định kỳ thất bại');
                  }
                }
              }
            },
  ...
  Future handleButtonSave(
    ...
    if (_moneyController.text.isEmpty) {
      ...
      if (response is RecurringPost) {
        ...
        showMessage1OptionDialog(
            this.context, 'Thêm giao dịch định kỳ thất bại');
      }
    }
  }
  ...
  Widget _selectDate() {
    return SizedBox(
      child: ListTile(
        ...
        subtitle: Text(
          isNotNullOrEmpty(optionTitle)
              ? optionTitle!
              : 'Chọn thời gian lặp lại',
          style: TextStyle(
            fontSize: 16,
            color: isNotNullOrEmpty(optionTitle) ? Colors.black : Colors.grey,
          ),
        ),
        trailing: const Icon(
          Icons.arrow_forward_ios,
  ...
  Future _getOptionFrequency() async {
    ...
    String frequencyName =
        (result.frequency.frequencyType == FrequencyType.weekday)
            ? dayWeek
